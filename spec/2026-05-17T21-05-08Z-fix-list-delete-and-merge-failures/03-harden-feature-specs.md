...
- Edit `spec/support/shared_examples/lists.rb` only where existing `sleep 1` (or equivalent unconditional sleeps) workaround flakiness around the confirm dialog mount. Known smell sites: lines 149 and 197 (per turn-1 notes).
...
- [ ] Identify every `sleep` and unconditional `wait` in `spec/support/shared_examples/lists.rb` introduced as a workaround for confirm-dialog timing. At minimum address the two at lines 149 and 197.
...
- [ ] No `sleep` calls remain in `spec/support/shared_examples/lists.rb` for the confirm-dialog code paths exercised by the 9 examples.
